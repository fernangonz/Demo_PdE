# -*- coding: utf-8 -*-
"""Punto de entrada del modelo PI_PRECIPITACION (exceso de precipitacion / ELO)."""
    ...
    for numero, fila_rel in enumerate(modos, start=1):
        ...
        if debe_omitir_im(
            ...
            pasos_totales.append(PasoResultado(
                numero=5,
                nombre=f"Iteracion por Modos de fallo (IM={numero}) - omitido",
                excel="Configuracion de impactos no factibles",
        ...
        pestana_ref = next(
            (
                getattr(i, _ATTR_PESTANA, "")
                for i in indicadores
                if getattr(i, _ATTR_PESTANA, "")
            ),
            "",
        )
        ...
        if faltantes or len(filas_ok) != len(indicadores):
            ...
            motivo = (
                ...
                f"Faltan: {falt_txt}. Encontrados: {enc_txt}."
            )
            iteraciones.append(_iteracion_error(
                numero=numero,
                etiqueta_im=etiqueta_im,
